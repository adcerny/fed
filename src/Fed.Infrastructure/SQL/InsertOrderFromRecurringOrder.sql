...
SELECT 
    @OrderId, @ShortId, [ro].[Id], [ro].[Name], @DeliveryDate, GETDATE(), [ro].[WeeklyRecurrence],
    ...
    [co].[PaymentMethodId], [cu].[SplitDeliveriesByOrder], [ro].[IsFree] 
FROM [dbo].[RecurringOrders] AS [ro]  
INNER JOIN [dbo].[Timeslots] AS [ts] ON [ro].[TimeslotId] = [ts].[Id] 
INNER JOIN [dbo].[Hubs] AS [hu] ON [ts].[HubId] = [hu].[Id] 
INNER JOIN [dbo].[Contacts] AS [co] ON [ro].[ContactId] = [co].[Id] 
INNER JOIN [dbo].[Customers] AS [cu] ON [co].[CustomerId] = [cu].[Id] 
INNER JOIN [dbo].[DeliveryAddresses] AS [da] ON [ro].[DeliveryAddressId] = [da].[Id] 
INNER JOIN [dbo].[BillingAddresses] AS [ba] ON [ro].[BillingAddressId] = [ba].[Id] 
WHERE [ro].[Id] = @RecurringOrderId