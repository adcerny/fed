...
WHERE
	((TRY_CONVERT(UNIQUEIDENTIFIER, @CustomerId) IS NOT NULL AND TRY_CONVERT(UNIQUEIDENTIFIER, @CustomerId) = [cust].[Id])
	OR ([cust].[ShortId] = @CustomerId))
	AND [del].[DeliveryDate] >= @FromDate
